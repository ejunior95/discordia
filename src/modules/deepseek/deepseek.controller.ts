...
export class DeepseekController {
  ...
  async testMessage(
    ...
    try {
      ...
      const result = await this.deepseekService.execute('chat', body.question, history);

      await this.historyService.add('chat', userId, 'user', body.question);
      await this.historyService.add('chat', userId, 'assistant', result.response, 'deepseek');
